...
# DADA2 pipeline
For this pipeline you need to open a RStudio session and allocate 8h, 50Gbb and 4 cores.
...
We'll use a bit less data for this step to speed up the process (default is 1e8).
...
And convert it to a sequence table and print the dimensions and the length distribution of the ASVs.
...
All the ASVs are now made and we can save the sequence table as an RDS file.

```{r "saveSeqtab", eval=FALSE}
saveRDS(seqtab.nochim, "seqtab.rds")
```


Then we can assign taxonomy to the ASVs. And save the taxonomy table as an RDS file.
